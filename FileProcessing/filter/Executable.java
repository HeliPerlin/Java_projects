...
public class Executable extends Filter {
    ...
    /**
     * constructs a new executable filter with the given arguments
     * @param answer indicates whether the filter should pass files that are executable or not
     * @param NotIsOn the negation indicator
     */
    public Executable(String answer, boolean NotIsOn){
        super(NotIsOn);
    ...
    public boolean accept(File pathname) {
        if (NotIsOn) {
            return !(pathname.canExecute() & answer) && pathname.isFile();
        }
        return (pathname.canExecute() & answer) && pathname.isFile();
    }
}
